...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variableDomainsDict[OBS0] = obsValue
    variableDomainsDict[OBS1]= obsValue
    "*** END YOUR CODE HERE ***"
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for var in eliminationOrder:
            CPTs, newFactor = joinFactorsByVariable(CPTs, var)
            if len(newFactor.unconditionedVariables()) > 1:
                elimFactor = eliminate(newFactor, var)
                CPTs.append(elimFactor)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        randVal = random.random() * totalSum
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        if noisyDistance is not None and ghostPosition != jailPosition:
            ...
            observationDistribution = busters.getObservationProbability(noisyDistance, trueDistance) # P(noisyDistance | trueDistance)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions: # all possible ghost positions
            observationProb = self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)
            updatedBelief = self.beliefs[ghostPosition] * observationProb
            self.beliefs[ghostPosition] = updatedBelief
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # get all the steps into each position we use old beliefs
        newPositionProbs = {} # holds each new position as a key and a list of probabilities from old positions to the new position
        for oldPos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, oldPos) # we get the new position and their probabilities
            for (newPos, newProb) in newPosDist.items():
                if newPos in newPositionProbs:
                    newPositionProbs[newPos].append((oldPos, newProb))
                else:
                    newPositionProbs[newPos] = [(oldPos, newProb)]

        # now we update each belief
        oldBeliefs = self.beliefs.copy()
        for pos in self.allPositions:
            if pos not in newPositionProbs:
                self.beliefs[pos] = 0.0
                continue
            listOfProbs = newPositionProbs[pos]
            newBelief = 0
            for (oldPos, prob) in listOfProbs:
                newBelief += prob * oldBeliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        # raiseNotDefined()
        # fill self.particles evenly with as many particles as possible
        numParticlesPerPosition = self.numParticles // len(self.legalPositions)
        for position in self.legalPositions:
            self.particles += [position for _ in range(numParticlesPerPosition)]

        # check for any leftover particles and fill these as evenly as possible
        leftoverParticles = self.numParticles % len(self.legalPositions)
        positionIndex = 0
        while leftoverParticles > 0:
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles:
            if particle in beliefDistribution:
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # for each particle, get the probability of observation
        pacmanPosition = gameState.getPacmanPosition()
        jailPosition = self.getJailPosition()
        weightDistributon = DiscreteDistribution()
        for particle in self.particles:
            tempProb = self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
            if particle in weightDistributon:
                weightDistributon[particle] += tempProb
            else:
                weightDistributon[particle] = tempProb
        
        # print(weightDistributon)

        # check for edge case of zero weight
        if weightDistributon.total() == 0:
            self.initializeUniformly(gameState)
        else:
            # resample our particles
            newParticles = []
            for _ in range(self.numParticles):
                newParticles.append(weightDistributon.sample())
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # for each particle we find the possible next steps and sample from the resulting distribution
        newParticles = []
        for particle in self.particles:
